...
function shortcode_recent_posts($atts, $content = null) {
		...
		foreach($latest as $post) {
				...
				if ($custom_class!="services") {
					if ($thumb == 'true') {
						if ( has_post_thumbnail($post->ID) ){
								$output .= '<a href="'.get_permalink($post->ID).'" title="'.get_the_title($post->ID).'" class="thumb-wrap">';
						...
						$output .= '<strong class="post">'.$teampost.'</strong>';
						
						if($meta == 'true'){
								$output .= '<span class="meta">';
										$output .= __('Posted on ', 'theme20');
				...
				if ($excerpt_count >= 1) {
					...
					$output .= '</div>';
				} elseif ($excerpt_count == 0) {
					$output .= '<div class="excerpt">';
						$output .= get_the_excerpt();
					$output .= '</div>';
				}
				
				if (($custom_class=="services") || ($custom_class=="info")) {
					$output .= '<a href="'.get_permalink($post->ID).'" class="button" title="'.get_the_title($post->ID).'">read more</a>';
				}
...
function shortcode_audio($atts, $content = null) {
    ...
    if($mp3 !== false){
        ...
        _e("The URL you entered is not a .mp3 file.", "theme20");
...
function shortcode_video($atts, $content = null) {
    ...
    if($vimeo !== false){
        ...
        _e("You entered a video URL that isn't compatible with this shortcode.", "theme20");
